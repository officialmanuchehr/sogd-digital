      ...
      <div className="container-page">
        <FadeIn>
          ...
          <p className="mt-4 max-w-xl text-muted">
            Прозрачный процесс от первой встречи до запуска и поддержки.
          </p>
        </FadeIn>
        ...
        <div className="relative mt-20 hidden md:grid md:grid-cols-4 md:gap-8">
          <div
            aria-hidden
            className="absolute left-0 right-0 top-6 h-px bg-brand-gradient opacity-40"
            style={{ marginInline: "12.5%" }}
          />
          {processSteps.map((step) => (
            <div key={step.number} className="relative flex flex-col items-start">
              <div className="relative z-10 flex h-12 w-12 items-center justify-center rounded-full border border-line bg-ink font-display text-sm font-semibold text-gold">
                {step.number}
              </div>
        ...
        <div className="relative mt-14 flex flex-col gap-10 md:hidden">
          ...
          {processSteps.map((step) => (
            <div key={step.number} className="relative flex gap-5">
              <div className="relative z-10 flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-line bg-ink font-display text-sm font-semibold text-gold">
                {step.number}
              </div>
              <div className="pt-2">
